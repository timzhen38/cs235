...
void LinkedList<T>::rotate(int k)
{
   ...
   {
      getNodeAt(getLength()-1) -> setNext(getNodeAt(0)); //link the last element to first element

      head_ptr_ = getNodeAt(getLength()-1); //change the index of last element into first element

      getNodeAt(getLength()-1) -> setNext(nullptr); //cut off the link between i-1 and i
      rotate(k-1);
   }
}
...
void LinkedList<T>::invertRest(Node<T>* current_first_ptr)
{
   ...
   {
      invertRest(current_first_ptr->getNext()); //recursion
      (current_first_ptr->getNext())->setNext(current_first_ptr); //link the next node to the one before it
